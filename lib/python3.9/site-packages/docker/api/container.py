...
"""
        ...
        if until is not None:
            if utils.version_lt(self._version, '1.35'):
                raise errors.InvalidVersion(
                    'util is not supported for API version < 1.35'
                )
            if isinstance(until, datetime):
                params['util'] = utils.datetime_to_timestamp(until)
            elif (isinstance(until, int) and until > 0):
                params['util'] = until
            else:
                raise errors.InvalidArgument(
                    'util value should be datetime or positive int, '
    ...
    def wait(self, container, timeout=None, condition=None):
        """
        Block util a container stops, then return its exit code. Similar to
        ...
        Args:
            ...
            condition (str): Wait util a container state reaches the given
