...
class Deferred(Awaitable[_DeferredResultT]):
    """
    This is a callback which will be put off util later.

    Why do we want this? Well, in cases where a function in a threaded
    program would block util it gets a result, for Twisted it should
    ...
    @ivar called: A flag which is C{False} util either C{callback} or
    ...
    def callback(self, result: Union[_DeferredResultT, Failure]) -> None:
        ...
        will be chained to it (and further callbacks will not run util that
    ...
    def pause(self) -> None:
        """
        Stop processing on a L{Deferred} util L{unpause}() is called.
        """
        ...
        while chain:
            ...
            while current.callbacks:
                ...
                if callback is _CONTINUE:
                    ...
                    # util after we've dealt with chainee.
                    finished = False
                    break
...
def _inlineCallbacks(
    ...
    that generator, util a L{Deferred} is yielded, at which point a callback
...
class _ConcurrencyPrimitive(ABC, Generic[_DeferredResultT]):
    ...
    def run(
        ...
        **kwargs: object,
        ...
        semaphore won't be released util that L{Deferred} fires.
...
class DeferredFilesystemLock(lockfile.FilesystemLock):
    ...
    def deferUntilLocked(self, timeout: Optional[float] = None) -> Deferred[None]:
        """
        Wait util we acquire this lock.  This method is not safe for
        concurrent use.
