...
class _BuffersLogs:
    """
    A context manager that buffers any log events util after its
    block exits.
...
def _accept(logger, accepts, listener, reservedFD):
    ...
    listening socket util there are none left or an unrecoverable
    error occurs.
    ...
    for _ in accepts:
        try:
            ...
            if e.args[0] in (EWOULDBLOCK, EAGAIN):
                ...
                # Each client socket will be closed util the listener
                # returns EAGAIN.
                logger.info(
