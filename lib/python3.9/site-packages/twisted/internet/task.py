...
class LoopingCall:
    ...
    If C{f} returns a deferred, rescheduling will not take place util the
    ...
    def start(self, interval: float, now: bool = True) -> Deferred["LoopingCall"]:
        ...
        util the interval has elapsed before beginning.
    ...
    def _scheduleFrom(self, when: float) -> None:
        ...
        def howLong() -> float:
            # How long should it take util the next invocation of our
            ...
            if self.interval == 0:
                ...
                return 0
            # Compute the time util the next interval; how long has this call
            # been running for?
            assert self.starttime is not None
...
class CooperativeTask:
    ...
    def pause(self) -> None:
        """
        Pause this L{CooperativeTask}.  Stop doing work util
...
class Cooperator:
    ...
    iterator yields a L{Deferred} then work will pause util the
    ...
    def __init__(
        ...
        stepped as soon as they are added, or if they will be queued up util
...
def react(
    ...
    """
    Call C{main} and run the reactor util the L{Deferred} it returns fires or
